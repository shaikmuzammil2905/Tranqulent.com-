  ...
  return (
    <section className="py-20 lg:py-28 bg-white relative">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {/* 4 Pillars with Vertical Divider Lines on Desktop */}
        <motion.div
          initial={{ opacity: 0, y: 20 }}
          whileInView={{ opacity: 1, y: 0 }}
          viewport={{ once: true, margin: "-80px" }}
          transition={{ duration: 0.55 }}
          className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8 lg:gap-0 lg:divide-x lg:divide-slate-200"
        >
          {WHY_TRANQUELENT_PILLARS.map((pillar, idx) => (
            ...
            >
              {/* Pillar Icon */}
              <div className="w-12 h-12 rounded-xl bg-brand-light-grey text-brand-blue flex items-center justify-center mb-6 shadow-sm border border-slate-100">
                {getIcon(pillar.icon)}
              </div>

              {/* Pillar Title */}
              <h3 className="text-lg font-bold text-brand-dark-navy mb-3">
                {pillar.title}
              </h3>
